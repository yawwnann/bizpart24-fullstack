...
export default function AdminOrderDetailPage() {
  ...
  const handleUpdateShipping = async () => {
    ...
    } catch (error) { console.error("Failed to update shipping", error) }
  ...
  const handleUpdateTracking = async () => {
    ...
    try {
      ...
      if (response.data.success) setOrder(response.data.data)
    } catch (error) { console.error("Failed to update tracking", error) }
  ...
  const statusMap: Record<string, { label: string; cls: string }> = {
    menunggu_ongkir:     { label: "Menunggu Ongkir",     cls: "bg-gray-100 text-gray-600 border-gray-200"       },
        ...
        <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-100">
              <div className="px-5 py-3.5 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-gray-900">Konfirmasi Ongkos Kirim</p>
                </div>
                <p className="text-xs text-gray-400 mt-0.5 ml-7">Simpan untuk kirim tagihan ke pelanggan</p>
              </div>
              <div className="p-5">
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <Input
                      ...
                      onChange={(e) => { const raw = e.target.value.replace(/\D/g, ""); setShippingCost(raw) }}
                    />
                  </div>
                  <button
            ...
            <div className="bg-white rounded-xl border border-gray-100">
              <div className="px-5 py-3.5 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-gray-900">Verifikasi Bukti Pembayaran</p>
                </div>
                <p className="text-xs text-gray-400 mt-0.5 ml-7">Periksa bukti transfer yang dikirim pelanggan</p>
              </div>
              <div className="p-5">
                {order.paymentProof ? (
                  <div className="space-y-3">
                    <div className="rounded-lg overflow-hidden border border-gray-100 relative group cursor-pointer">
                      <img src={order.paymentProof} alt="Bukti Pembayaran" className="w-full object-contain max-h-56 bg-gray-50" />
                      <a
                        href={order.paymentProof} target="_blank" rel="noreferrer"
                    ...
                    >
                      {isUpdatingStatus ? "Memproses..." : "✓ Verifikasi & Tandai Diproses"}
                    </button>
                  </div>
                ) : (
          ...
          {order.status === "diproses" && (
            <div className="bg-white rounded-xl border border-gray-100">
              <div className="px-5 py-3.5 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-violet-100 text-violet-700 text-[10px] font-bold flex items-center justify-center">3</span>
                  <p className="text-sm font-semibold text-gray-900">Input Ekspedisi & Resi</p>
                </div>
                <p className="text-xs text-gray-400 mt-0.5 ml-7">Otomatis ubah status ke "Dikirim" & email ke pelanggan</p>
              </div>
              <div className="p-5">
                {order.courierType ? (
                  <div className="space-y-3">
          ...
          {order.status === "dikirim" && (
            <>
              {order.courierType && (
                <div className="bg-white rounded-xl border border-gray-100">
                  <div className="px-5 py-3.5 border-b border-gray-100">
                    <p className="text-sm font-semibold text-gray-900">Info Pengiriman</p>
                  </div>
              ...
              <div className="bg-white rounded-xl border border-emerald-100">
                <div className="px-5 py-3.5 border-b border-emerald-100">
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-bold flex items-center justify-center">4</span>
                    <p className="text-sm font-semibold text-gray-900">Tandai Pesanan Selesai</p>
                  </div>
                  <p className="text-xs text-gray-400 mt-0.5 ml-7">Konfirmasi manual jika barang sudah diterima pelanggan</p>
                </div>
                <div className="p-5">
                  <button
                  ...
                  >
                    {isUpdatingStatus ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Memproses...</> : "✓ Tandai Selesai"}
                  </button>
                  <p className="text-xs text-gray-400 text-center mt-2">atau tunggu pelanggan konfirmasi sendiri di /payment</p>
                </div>
              </div>
            </>
          )}
          ...
          {order.status === "selesai" && (
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-5">
              ...
              <p className="text-xs text-emerald-700 mt-1">Pesanan ini telah selesai.</p>
            </div>
          )}
          ...
          {order.paymentProof && order.status !== "menunggu_pembayaran" && (
            <div className="bg-white rounded-xl border border-gray-100">
              ...
              <div className="p-5">
                <div className="rounded-lg overflow-hidden border border-gray-100 relative group cursor-pointer">
                  <img src={order.paymentProof} alt="Bukti Pembayaran" className="w-full object-contain max-h-40 bg-gray-50" />
                  <a
                    href={order.paymentProof} target="_blank" rel="noreferrer"
                    className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-xs font-medium gap-1 transition-opacity"
                  >
          ...
          {order.status !== "selesai" && (
            <div className="bg-white rounded-xl border border-gray-100">
              <div className="px-5 py-3.5 border-b border-gray-100">
                <p className="text-sm font-semibold text-gray-900">Ganti Status Manual</p>
                <p className="text-xs text-gray-400 mt-0.5">Override untuk koreksi atau pembatalan</p>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium border transition-colors disabled:opacity-40 flex items-center justify-between
                        ${isActive
                          ? btn.danger ? "bg-red-50 border-red-200 text-red-700" : "bg-gray-900 border-gray-900 text-white"
                          : btn.danger ? "border-gray-200 text-red-500 hover:bg-red-50 hover:border-red-200" : "border-gray-200 text-gray-700 hover:bg-gray-50"
                        }`}
                      disabled={isActive || isUpdatingStatus}
                      onClick={() => handleUpdateStatus(btn.value)}
                    >
                      <span>{btn.label}</span>
                      {isActive && <span className={`w-1.5 h-1.5 rounded-full ${btn.danger ? "bg-red-500" : "bg-white"}`} />}
                    </button>
                  )
                })}
              </div>
